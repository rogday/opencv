...
using ONNXConfParams = std::tuple<TestCaseInput, std::tuple<Backend, Target>>;
...
std::string printOnnxConfParams(const testing::TestParamInfo<ONNXConfParams>& params)
{
    ...
    Backend backend;
    Target target;
    auto net_params = std::tie(backend, target);
    std::tie(test_case, net_params) = params.param;
...
std::vector<TestCaseInput> readTestCases()
{
    ...
    for (const auto& test_case : testConformanceConfig)
    {
        TestCaseInput input;
...
public:
    ...
    {
        auto net_params = std::tie(backend, target);
        std::tie(test_case, net_params) = GetParam();
